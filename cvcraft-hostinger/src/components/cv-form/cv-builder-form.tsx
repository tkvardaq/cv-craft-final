...
import { ShareButton } from "../builder/share-button";
...
export function CvBuilderForm({ initialId, initialTemplate, isPremium = false }: { initialId?: string; initialTemplate?: string; isPremium?: boolean }) {
  ...
  useEffect(() => {
    const loadCv = async () => {
      ...
      try {
        ...
        toast.error("Couldn't load your CV. Starting with a blank draft.");
      } finally {
        setIsInitialLoadComplete(true);
      }
    };
  ...
  const onSubmit = useCallback(async (data: CV) => {
    ...
    try {
      ...
      toast.error("Couldn't save your changes. Retrying on next edit.");
    } finally {
  ...
  const handleAnalyzeJd = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error("Couldn't analyze that job description. Please try again.");
      }
    } catch {
      toast.error("Network error while analyzing job description.");
    } finally {
  ...
  return (
    <FormProvider {...methods}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className={cn(
          "lg:col-span-4 space-y-8 lg:sticky lg:top-24 pb-12",
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {jdAnalysis.keywords.slice(0, 10).map((k) => (
                      <span key={k} className="px-3 py-1 bg-white/5 rounded-full text-[11px] text-slate-300 border border-white/5 hover:border-royal-gold/30 hover:text-white transition-colors cursor-default">
                        {k}
                      </span>
                    ))}
                  </div>
