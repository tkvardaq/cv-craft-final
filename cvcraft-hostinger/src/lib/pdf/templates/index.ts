...
export interface TemplateDefinition {
  ...
  name: string;
  description: string;
  component: React.ComponentType<{ cv: CV; isPremium?: boolean }>;
  previewUrl?: string;
}
...
export function getTemplate(id?: string | null): TemplateDefinition {
  const fallback = templates.find((t) => t.id === "professional") ?? templates[0];
